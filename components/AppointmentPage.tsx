...
const AppointmentPage: React.FC<AppointmentPageProps> = ({ onOpenAdmin }) => {
  ...
  // Generated Dates for Visual Picker (Next 14 days, excluding Sunday)
  const availableDates = useMemo(() => {
    const dates = [];
    const today = new Date();
    let current = new Date(today);
    
    // Generate next 14 valid days
  ...
  const handleDateScroll = (direction: 'left' | 'right') => {
    if (dateScrollRef.current) {
        // A date tile is w-20 (80px) + gap-3 (12px) = 92px.
        // Let's scroll by about 4 tiles. 4 * 92 = 368px.
        const scrollAmount = direction === 'left' ? -368 : 368;
        ...
        <AnimatePresence mode="wait">
            {!isSent ? (
                <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className="w-full max-w-[1400px] bg-white/5 backdrop-blur-xl border border-white/10 p-4 md:p-12 relative overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.5)]"
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                            <motion.div key="step1" variants={contentVariants} initial="hidden" animate="visible" exit="exit" className="space-y-8">
                                ...
                                {/* SCROLLABLE CONTAINER WITH MYSTERIOUS SIDE HIGHLIGHTS */}
                                <div className="relative">
                                    ...
                                    {/* LEFT SIDE GLOW HIGHLIGHT */}
                                    <div className="absolute left-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-white/50 to-transparent opacity-30 z-20 pointer-events-none shadow-[0_0_15px_rgba(255,255,255,0.3)]" />
                                    
                                    {/* RIGHT SIDE GLOW HIGHLIGHT */}
                                    <div className="absolute right-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-white/50 to-transparent opacity-30 z-20 pointer-events-none shadow-[0_0_15px_rgba(255,255,255,0.3)]" />

                                    {/* Vertical Efficient Grid - INCREASED HEIGHT */}
                                    <motion.div 
                                        variants={gridContainerVariants}
                                        initial="hidden"
                                        animate="visible"
                                        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[75vh] overflow-y-auto px-4 py-8 custom-scrollbar relative z-0"
                                    >
                                        {availableCars.map(car => {
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    className={`relative cursor-pointer group rounded-sm overflow-hidden transition-all duration-300 border flex flex-col ${
                                                        isSelected 
                        ...
                        {step === 2 && (
                            <motion.div key="step2" variants={contentVariants} initial="hidden" animate="visible" exit="exit" className="space-y-10">
                                ...
                                <div className="max-w-3xl mx-auto space-y-8">
                                    ...
                                    {/* Date Selection - Horizontal Tiles */}
                                    <div className="space-y-4">
                                        ...
                                        <div className="relative flex items-center">
                                            <button 
                                                ...
                                                className="hidden md:block absolute -left-8 z-10 p-2 border border-white/10 hover:border-white hover:bg-white hover:text-black text-white rounded-full transition-all duration-300"
                                                aria-label="Vorige datums"
                                            >
                                                <ChevronLeft size={16} />
                                            </button>
                                            <div ref={dateScrollRef} className="flex overflow-x-auto gap-3 pb-4 no-scrollbar justify-start w-full">
                                            ...
                                            <button 
                                                ...
                                                className="hidden md:block absolute -right-8 z-10 p-2 border border-white/10 hover:border-white hover:bg-white hover:text-black text-white rounded-full transition-all duration-300"
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                    {availableSlots.map(slot => (
                                                        <motion.button
                                                            ...
                                                            whileTap={{ scale: 0.95 }}
                                                            className={`py-3 px-2 border text-xs font-bold uppercase tracking-wider transition-all duration-300 rounded-sm ${
                                                                formData.time === slot 
